...
| <a name="input_routing_policy_name"></a> [routing\_policy\_name](#input\_routing\_policy\_name) | The name of the IAM policy granting the subnet router permission to disable its own source/dest<br/>  check and manage routes. Only created when `source_dest_check = false` or routes are configured.<br/>  Multiple instances of this module can be used in the same account by setting a unique `routing_policy_name` for each instance. | `string` | `"routing"` | no |
